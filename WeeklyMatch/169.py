...
class Solution(object):
    def getAllElements(self, root1, root2):
        ...
        lorder = []
        self.inOrder(root1, lorder)
        rorder = []
        self.inOrder(root2, rorder)
...
class Solution2(object):
    def canReach(self, arr, start):
        flag = [0] * len(arr)
        return self.canJump(arr, start, flag)

    def canJump(self, arr, start, flag):
        if start > len(arr) - 1 or start < 0 or flag[start] == 1:
            return False
        ...
        return self.canJump(arr, start + arr[start], flag) or self.canJump(arr, start - arr[start], flag)
...
if __name__ == '__main__':
    ...
    start = 313
    print(ps.canReach(arr, start))
    print(arr[arr[start] + 31])
    print(arr[arr[start] - 31])
